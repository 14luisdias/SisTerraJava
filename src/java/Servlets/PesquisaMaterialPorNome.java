 ...
 * @author Qualidade
 */
@WebServlet(name = "PesquisaMaterialPorNome", urlPatterns = {"/pesquisaMaterialPorNome"})
public class PesquisaMaterialPorNome extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        //processRequest(request, response);
            if (("".equals(request.getParameter("codigo")))&&(!"".equals(request.getParameter("nome")))){   
                    ...
                    String nomeMaterial = request.getParameter("nome");
                    MaterialImpl materialDao = new MaterialImpl();
                    //pesquisei pelo material
                    List<Material> material = materialDao.findByNome(nomeMaterial);
        
                    request.setAttribute("material", material);
                    RequestDispatcher dispatcher;
                    dispatcher = request.getRequestDispatcher("PesquisaMaterialPorNome.jsp");
                    dispatcher.forward(request, response);
            }
            if ((!"".equals(request.getParameter("codigo")))&&("".equals(request.getParameter("nome")))){
                int cod = Integer.valueOf(request.getParameter("codigo"));
                 response.sendRedirect("pesquisaMaterialporid?id="+cod);
